...
def read_fits_soar(path: str, header: bool = False) -> DataBz:
    ...
    image: np.ndarray[np.float64, np.dtype[np.float64]] = getdata(path, ext=False)  # type: ignore
    ...
    x_start = np.int64(input("First pixel x axis: "))
    x_last = np.int64(input("Last pixel x axis: "))
    y_start = np.int64(input("First pixel y axis: "))
    y_last = np.int64(input("Last pixel y axis: "))
    x_start = np.int64(400)
    x_last = np.int64(1200)
    y_start = np.int64(500)
    y_last = np.int64(1000)
    ...
    hdr = image[0].header  # the primary HDU header
    dist = hdr["DSUN_OBS"]
